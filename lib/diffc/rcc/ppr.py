...
def _reverse_channel_encode(mu_q_in, K, alpha = 2.0, shared_seed=0):
...
def reverse_channel_encode(mu_q, K=None, shared_seed=0, alpha=2.0):
    diff = (mu_q).astype(np.float32)  # Convert to float32
    seed, sample = _reverse_channel_encode(diff, K, alpha=alpha, shared_seed=shared_seed)
    return seed, sample


def reverse_channel_decode(dim, seed, shared_seed=0):
    """
    Given an isotropic laplacian with b = 1 centered at mu_q,
